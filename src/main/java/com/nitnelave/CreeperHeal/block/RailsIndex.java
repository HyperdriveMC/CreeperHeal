...
public class RailsIndex {
  ...
  static {
    ...
    Bukkit.getScheduler()
        .scheduleSyncRepeatingTask(CreeperHeal.getInstance(), RailsIndex::cleanUp, 500, 7200);
  }
   ...
   * @return Whether the location is next to a block whose update is prevented.
   */
  public static boolean isUpdatePrevented(Block block) {
    if (!Tag.RAILS.isTagged(block.getType())) return false;
    return railsIndex.containsKey(block);
  }

  /**
   * Add the location to the list of blocks that shouldn't be updated. The block's updates are
   * prevented until after 200 times the block per block replacement interval.
   *
   * @param block The block.
   */
  public static void putUpdatePrevention(Block block) {
    if (CreeperConfig.getBool(CfgVal.RAIL_REPLACEMENT)) railsIndex.put(block, new Date());
  }

  private static void cleanUp() {
    if (!CreeperConfig.getBool(CfgVal.RAIL_REPLACEMENT)) return;
