...
public class CreeperBurntBlock {
   ...
   * Constructor.
   *
   * @param now The date the block has burnt.
   * @param block The burnt block.
   */
   ...
   * Constructor.
   *
   * @param now The date at which the block burnt.
   * @param state The blockState to be represented.
   */
   ...
   * Replace the block.
   *
   * @param shouldDrop If true, in case of failure, the replacement isn't postponed, but the block
   *     is dropped.
  ...
  public boolean replace(boolean shouldDrop) {
    ...
    if (event.isCancelled()) return event.shouldDrop();
   ...
   * @throws NullPointerException If the block is invalid.
   */
   ...
   *
   * @return The block's attaching face, or BlockFace.SELF if the block is invalid.
   */
  @SuppressWarnings("unused")
  public BlockFace getAttachingFace() {
    if (block != null) return block.getAttachingFace();
   ...
   *
   * @param delay The amount of time to postpone by, in sec.
  ...
  public boolean checkReplace() {
    if (timer.checkReplace()) {
      if (replace(false)) replaced = true;
      else postPone(CreeperConfig.getInt(CfgVal.WAIT_BEFORE_HEAL));
      return true;
    }
    return false;
  }
