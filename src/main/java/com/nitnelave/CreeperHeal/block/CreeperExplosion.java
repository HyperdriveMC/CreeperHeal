...
/**
 * Represents an explosion, with the list of blocks destroyed, the time of the explosion, and the
 * radius.
 *
 * @author nitnelave
 */
public class CreeperExplosion {
  ...
  /**
   * Add blocks to an explosion, and reset the timer to the time of the last explosion.
   *
  ...
  public void addBlocks(List<Block> blocks, Location newLoc) {
    ...
    loc =
        new Location(
            loc.getWorld(),
            (locWeight * loc.getX() + newLoc.getX()) / (locWeight + 1),
            (locWeight * loc.getY() + newLoc.getY()) / (locWeight + 1),
            (locWeight * loc.getZ() + newLoc.getZ()) / (locWeight + 1));
    locWeight++;
    checked.clear();
    recordBlocks(blocks);
    if (CreeperConfig.getBool(CfgVal.EXPLODE_OBSIDIAN)) checkForObsidian();
  ...
  /**
   * Get the radius of the explosion (i.e. the distance between the location and the furthest
   * block).
   *
  ...
  private void checkForObsidian() {
    ...
    for (int i = loc.getBlockX() - radius; i < loc.getBlockX() + radius; i++)
      for (int j = Math.max(0, loc.getBlockY() - radius);
          j < Math.min(w.getMaxHeight(), loc.getBlockY() + radius);
          j++)
        for (int k = loc.getBlockZ() - radius; k < loc.getBlockZ() + radius; k++) {
          Location l = new Location(w, i, j, k);
          if (l.distance(loc) > radius) continue;
          Block b = l.getBlock();
          if (isObsidianLike(b.getType(), table)
              && ThreadLocalRandom.current().nextDouble() < chance) recordBlock(b);
        }
  }
  ...
  /**
   * Record one block and remove it. If it is protected, add to the replace-immediately list. Check
   * for dependent blocks around.
   *
  ...
  public void recordBlock(Block block) {
    ...
    if (creeperBlock == null) return;

    ShortLocation location = new ShortLocation(creeperBlock.getLocation());

    if (checked.contains(location)) return;

    creeperBlock.record(checked);

    if ((CreeperConfig.getBool(CfgVal.PREVENT_CHAIN_REACTION)
            && creeperBlock.getType().equals(Material.TNT))
    ...
    if (!world.isBlackListed(creeperBlock.getType())) {
      ...
      creeperBlock.getDependentNeighbors().stream()
          .filter(NeighborBlock::isNeighbor)
   ...
   *
   * @param replaceable The Replaceable to add.
   */
  ...
  /**
   * Check if the explosion has blocks to repair, and repair them (or one of them in case of block
   * per block).
   *
   * @return False if the explosion has not started its replacements yet because it is not time.
   */
  public boolean checkReplace() {
    if (timer.isTimed()) return true;
    if (timer.checkReplace()) {
      if (CreeperConfig.getBool(CfgVal.BLOCK_PER_BLOCK)) replace_one_block();
      else replace_blocks(true, CHBlockHealReason.EXPLOSION);
      return true;
    }
    return false;
  }
  ...
  /**
   * Get whether the explosion has started replacing (only in block per block mode).
   *
   * @return True if the explosion has started replacing blocks
   */
  public boolean hasStartedReplacing() {
    if (timer.isTimed()) return false;
