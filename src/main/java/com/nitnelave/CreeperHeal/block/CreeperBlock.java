...
/**
 * Represents a block that can be replaced. Every special type of block derives from this class, and
 * can only be constructed by using the newBlock method.
 *
 * @author nitnelave
 */
public class CreeperBlock implements Replaceable {

  public static final BlockFace[] CARDINALS = {
    BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST, BlockFace.NORTH, BlockFace.UP, BlockFace.DOWN
  };
  /*
   ...
   * constructor.
   *
   * @param state The block to be represented.
  ...
  public static CreeperBlock newBlock(BlockState state) {
    ...
    if (state instanceof ShulkerBox) return new CreeperShulkerBox((ShulkerBox) state);
    if (state instanceof Container) return new CreeperContainer((Container) state);
    if (state instanceof Banner) return new CreeperBanner((Banner) state);
    if (state instanceof Jukebox) return new CreeperJukebox((Jukebox) state);
    if (state.getType().hasGravity()) return new CreeperPhysicsBlock(state);

    BlockData data = state.getBlockData();

    if (data instanceof Bed) return new CreeperBed(state);
  ...
  private static boolean isDependentDown(Material type) {
    if (Tag.SAPLINGS.isTagged(type)
        ...
        || CreeperTag.STANDING_BANNERS.isTagged(type)
  ...
  public static boolean isDependent(Material type) {
    return DEPENDENT_BLOCKS.contains(type)
   ...
   *
   * @param forced If false, the block will have a chance to drop, according to the configuration
   *     value of the drop chance. If true, the block drops every time.
  ...
  public boolean drop(boolean forced) {
    if (forced || CreeperConfig.shouldDrop()) {
      ...
      for (ItemStack itemStack : drop) world.dropItemNaturally(location, itemStack);
      return true;
    }
    return false;
  }
  ...
  public final boolean replace(boolean shouldDrop) {
    if (checkForDrop()) return true;

    if (!shouldDrop && isDependent(getType())) {
      BlockFace attachingFace = getAttachingFace();
      if (attachingFace != BlockFace.SELF
          && isEmpty(getBlock().getRelative(attachingFace).getType())) {
        return false;
      }
    }
  ...
  boolean checkForDrop() {
    ...
    if (!CreeperConfig.getBool(CfgVal.OVERWRITE_BLOCKS) && !isEmpty(type)) {
      if (CreeperConfig.getBool(CfgVal.DROP_DESTROYED_BLOCKS)) drop(true);
      return true;
    } else if (CreeperConfig.getBool(CfgVal.OVERWRITE_BLOCKS)
        && !isEmpty(type)
        && CreeperConfig.getBool(CfgVal.DROP_DESTROYED_BLOCKS)) {
      CreeperBlock b = CreeperBlock.newBlock(block.getState());
      if (b == null)
        throw new IllegalArgumentException("Null block for: " + block.getType());
      b.drop(true);
      b.remove();
    }
    return false;
  ...
  /**
   * Get the list of blocks that are possibly dependent on this block. To check if they really are,
   * simply check that neighborBlock.isNeighbor() is true.
   *
