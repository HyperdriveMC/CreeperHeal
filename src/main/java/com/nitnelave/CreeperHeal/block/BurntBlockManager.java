...
public abstract class BurntBlockManager {
  ...
  public static void init() {
    if (CreeperConfig.getInt(CfgVal.WAIT_BEFORE_BURN_AGAIN) > 0) recentlyBurnt = new HashMap<>();
    if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex = new NeighborFire();

    Bukkit.getScheduler()
        .scheduleSyncRepeatingTask(
            CreeperHeal.getInstance(), BurntBlockManager::cleanUp, 300, 7200);

    Bukkit.getScheduler()
        .runTaskTimer(CreeperHeal.getInstance(), BurntBlockManager::replaceBurnt, 0, 20);
  }

  /**
   * Force immediate replacement of all blocks burnt in the specified world
   *
   * @param worldConfig The world in which to replace the blocks.
   */
  public static void forceReplaceBurnt(WorldConfig worldConfig) {
    ...
    burntList.removeIf(
        burnt -> {
          ...
          if (waitBeforeBurn > 0) recentlyBurnt.put(burnt.getLocation(), time);
          if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex.removeElement(burnt);
  ...
  public static void forceReplaceBurnt() {
    ...
    if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex.clear();
    ...
    while (iter.hasNext()) {
      ...
      if (cBlock.checkReplace()) {
        if (cBlock.wasReplaced()) {
          iter.remove();
          if (waitBeforeBurn > 0) recentlyBurnt.put(cBlock.getLocation(), time);
          if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex.removeElement(cBlock);
        }
      } else break;
    }
  }
   ...
   *
   * @param block The block to be recorded.
   */
  public static void recordBurntBlock(Block block) {
    ...
    for (NeighborBlock neighbor : b.getDependentNeighbors())
      if (neighbor.isNeighbor())
        recordBurntBlock(
            new CreeperBurntBlock(
                new Date(new Date().getTime() + 100), neighbor.getBlock().getState()));
    recordBurntBlock(new CreeperBurntBlock(new Date(), b));
  }

  /**
   * Add a block to the list of burnt blocks to be replaced, and remove it from the world.
   *
  ...
  public static void recordBurntBlock(CreeperBurntBlock block) {
    if (block.getBlock() != null) {
      burntList.add(block);
      if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex.addElement(block);
      block.remove();
    }
  }
  ...
  public static boolean wasRecentlyBurnt(Block block) {
    if (CreeperConfig.getInt(CfgVal.WAIT_BEFORE_BURN_AGAIN) <= 0) return false;
    Date d = recentlyBurnt.get(block.getLocation());
    return d != null && d.after(new Date());
  }

  /** Clean up the block lists, remove the useless blocks. Do not use when in light weight mode. */
  private static void cleanUp() {
    if (CreeperConfig.getBool(CfgVal.LEAVES_VINES)) fireIndex.clean();
