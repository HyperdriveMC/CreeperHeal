...
public class CreeperListener implements Listener {
  ...
  /**
   * Listener for the EntityExplodeEvent. Record when appropriate the explosion for later
   * replacement.
   *
   * @param event The EntityExplode event.
   */
  ...
  public void onBlockExplode(BlockExplodeEvent event) {
    ...
    if (!FactionHandler.shouldIgnore(event.blockList(), world)) {
      if (!world.isAbove(event.getBlock().getLocation())) return;
  ...
  /**
   * Listener for the HangingBreakEvent. If appropriate, the hanging is recorded to be replaced
   * later on.
   *
  ...
  /**
   * Listener for the EntityChangeBlockEvent. Check for Endermen picking up blocks.
   *
   * @param event The EntityChangeBlock event.
   */
  ...
  public void onEntityChangeBlock(EntityChangeBlockEvent event) {
    ...
    if (event.getEntityType() == EntityType.SILVERFISH
      ...
      Bukkit.getScheduler()
          .runTask(CreeperHeal.getInstance(), new ReplaceSilverfishBlock(event.getBlock()));
    else if (event.getEntity().getType() == EntityType.ENDERMAN) {
      WorldConfig world = CreeperConfig.getWorld(event.getBlock().getWorld());
      if (world.getBool(WCfgVal.ENDERMAN)) event.setCancelled(true);
    }
  }
   ...
   *
   * @param event The EntityDamagedByEntity event.
   */
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
   ...
   *
   * @param event The EntityBreakDoor event.
   */
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
