...
public class GriefListener implements Listener {
  ...
  /**
   * Listener for the BlockPlaceEvent. If the player does not have the rights to place a block, the
   * event is cancelled, and the appropriate warnings are fired.
   *
  ...
  public void onBlockPlace(BlockPlaceEvent event) {
    ...
    if (event.getBlockPlaced().getType() == Material.TNT
      ...
      if (blocked) {
        ...
        CreeperLog.logInfo(
            "Blocked placement of a " + event.getBlockPlaced().getType() + " at ", 1);
      }
      ...
      if (world.getBool(WCfgVal.WARN_BLACKLIST))
        CreeperMessenger.warn(
            CreeperPlayer.WarningCause.BLACKLIST,
            player,
            blocked,
  ...
  /**
   * Listener for the BlockIgniteEvent. If fire spreading or fire from lava is disabled, cancel the
   * event.
   *
  ...
  /**
   * Listener for the EntityDamageByEntityEvent. Control PVP and check for destroyed paintings.
   *
      ...
      switch (event.getCause()) {
        case ENTITY_ATTACK:
          ...
          if (event.getEntity() instanceof Player
              ...
              && attacker instanceof Player) offender = (Player) attacker;
          break;
        case PROJECTILE:
        case MAGIC:
          ...
          if (damager instanceof Projectile) {
            ...
            if (source instanceof Player) offender = (Player) source;
          }
          break;
        default:
      }
      if (offender != null
          && !offender.equals(attacked)
   ...
   *
   * @param event The PlayerBucketEmpty event.
   */
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
  ...
  /**
   * Listener for the PlayerInteractEvent. Check for monster egg use, and block ignition.
   *
   * @param event The PlayerInteract event.
   */
  @EventHandler(priority = EventPriority.NORMAL, ignoreCancelled = true)
  ...
  /**
   * Listener for the PlayerJoinEvent. Add when appropriate to the warning list.
   *
  ...
  /**
   * Listener for the PlayerQuitEvent. Remove when appropriate from the warning list.
   *
   * @param event The PlayerQuit event.
   */
  @EventHandler(priority = EventPriority.MONITOR)
