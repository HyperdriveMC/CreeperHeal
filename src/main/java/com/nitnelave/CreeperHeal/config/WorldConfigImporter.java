...
abstract class WorldConfigImporter {
  ...
  private static boolean enderman,
      ...
      dragons,
      wither,
      ignoreFactionsWilderness,
      ignoreFactionsTerritory;
  ...
  private static boolean getStringBoolean(
      String path, String defaultValue, YamlConfiguration configFile) {
    String result;
    try {
      result = configFile.getString(path, defaultValue).trim().toLowerCase();
    } catch (Exception e) {
      CreeperLog.warning(
          "[CreeperHeal] Wrong value for " + path + " field. Defaulting to " + defaultValue + ".");
