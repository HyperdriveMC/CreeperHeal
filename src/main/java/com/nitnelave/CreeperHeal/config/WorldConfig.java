...
public class WorldConfig {
  ...
  /**
   * Main constructor. Load the config from the file, or create a default one if needed.
   *
  ...
  void load() {
    worldFolder.mkdirs();
    if (!configFile.exists()) FileUtils.copyJarConfig(configFile, "world.yml");
    if (!advancedFile.exists()) FileUtils.copyJarConfig(advancedFile, "world-advanced.yml");
    if (!griefFile.exists()) FileUtils.copyJarConfig(griefFile, "world-grief.yml");
    if (!replaceFile.exists()) FileUtils.createNewFile(replaceFile);
    try {
    ...
    if (isRepairTimed())
      Bukkit.getScheduler()
          .scheduleSyncRepeatingTask(CreeperHeal.getInstance(), this::checkReplaceTime, 200, 1200);
  }
  ...
  private void checkReplaceTime() {
    ...
    if (((Math.abs(getRepairTime() - time) < 600)
        || (Math.abs(Math.abs(getRepairTime() - time) - 24000)) < 600)) {
  ...
  void save() {
    for (ConfigValue<Boolean> v : booleans.values()) v.write();
    repairTime.write();
    replaceLimit.write();
   ...
   * @throws NullPointerException If the key does not represent a boolean.
   */
  ...
  /**
   * Get whether the location is above the replace limit defined in this world, if height
   * replacement is enabled.
   *
   * @param loc The location to test.
   * @return Whether the location is above the limit, or true if height replacement is not enabled.
   */
  ...
  public boolean isProtected(Block block) {
    return protectList.getValue().contains(block.getType())
        || (block.getState() instanceof InventoryHolder
            && CreeperConfig.getBool(CfgVal.REPLACE_PROTECTED_CHESTS)
            && PluginHandler.isProtected(block));
  }

  /**
   * Get whether any grief protection feature is enabled, thus requiring to listen to the grief
   * events.
   *
  ...
  public boolean hasGriefProtection() {
    return getBool(WCfgVal.BLOCK_LAVA)
        ...
        || (!griefPlaceList.getValue().isEmpty()
            && (getBool(WCfgVal.GRIEF_BLOCK_BLACKLIST) || getBool(WCfgVal.WARN_BLACKLIST)));
  }

  /**
   * Get whether the block is in the grief blackList (or not in the whitelist, if the whitelist is
   * used).
   *
   * @param block The block to test.
   ...
   * @throws NullPointerException If the key is not a valid boolean configuration value.
   */
  public void setBool(WCfgVal key, boolean value) throws NullPointerException {
    ConfigValue<Boolean> v = booleans.get(key.getKey());
    if (v == null) throw new NullPointerException("Unknown config key path : " + key.getKey());
    v.setValue(value);
  }
   ...
   * @param key The key.
   * @param value The value.
   * @throws NullPointerException If the key is not a valid Integer configuration value.
   */
  ...
  /**
   * Get whether a block is blacklisted for replacement (i.e. should not be replaced).
   *
  ...
  public boolean isBlackListed(Material material) {
    ...
    else return replaceBlackList.getValue().contains(material);
  }

  public void getReplacement(BlockState state) {
    String key = state.getType() + ";0";
    ...
    if (material == null) {
      CreeperLog.warning(
          "Invalid block type for " + key + " in world " + state.getWorld().getName());
      return;
    }
