...
class ConfigUpdater {
  ...
  private static boolean blockPerBlock,
      teleportOnSuffocate,
      dropDestroyedBlocks,
      crackDestroyedBricks,
      replaceProtectedChests,
      overwriteBlocks,
      preventBlockFall,
      lightweightMode,
      logWarnings,
      preventChainReaction,
      explodeObsidian,
      debug;
  ...
  private static void from4() throws IOException, InvalidConfigurationException {
    ...
    try {
      ...
      CreeperLog.warning(
          "[CreeperHeal] Wrong value for replacement method field. Defaulting to block-per-block.");
    ...
    if (!tmp_str.equalsIgnoreCase("no")
        && !tmp_str.equalsIgnoreCase("lwc")
        && !tmp_str.equalsIgnoreCase("all"))
      CreeperLog.warning("[CreeperHeal] Wrong value for chest protection field. Defaulting to no.");
    else if (tmp_str.equals("all") || tmp_str.equals("lwc")) replaceProtectedChests = true;

    configFile.delete();
  }

  private static void from5() throws IOException, InvalidConfigurationException {
    ...
    replaceProtectedChests =
        replaceAllChests || config.getBoolean("replacement.ignore-chests.protected", false);
  ...
  private static void from6() throws IOException, InvalidConfigurationException {
    ...
    replaceProtectedChests =
        replaceAllChests || config.getBoolean("ignore-chests.protected", false);
