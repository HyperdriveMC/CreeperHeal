...
public class CreeperCommandManager implements CommandExecutor {
  ...
  public static void registerCommands() {
    ...
    if (commandMap != null) commandMap.register("_", com);
  }
  ...
  public boolean onCommand(
    ...
    if (args.length != 0) {
      ...
      if (w != null) currentWorld = CreeperConfig.getWorld(args[args.length - 1]);
      ...
      if (cmd.equalsIgnoreCase("creeper"))
        ...
        integerCmd(
            CfgVal.WAIT_BEFORE_HEAL,
            args,
            "Sets the interval before replacing a block destroyed in an explosion",
            "seconds",
            sender);
      else if (cmd.equalsIgnoreCase("waitBeforeHealBurnt") || cmd.equalsIgnoreCase("wbhb"))
        integerCmd(
            CfgVal.WAIT_BEFORE_HEAL_BURNT,
            args,
            "Sets the interval before replacing a burnt block",
            "seconds",
            sender);
      else if (cmd.equalsIgnoreCase("blockPerBlockInterval") || cmd.equalsIgnoreCase("bpbi")) {
        integerCmd(
            CfgVal.BLOCK_PER_BLOCK_INTERVAL,
            args,
            "Sets the interval between block replacement",
            "ticks",
            sender);
      ...
      else if (cmd.equalsIgnoreCase("healNear")) healNear(sender, args);
      else if (cmd.equalsIgnoreCase("reload")) CreeperConfig.load();
      else if (cmd.equalsIgnoreCase("help")) sendHelp(sender);
      else if (cmd.equalsIgnoreCase("on")) enable(true, currentWorld, sender);
      else if (cmd.equalsIgnoreCase("off")) enable(false, currentWorld, sender);
      else {
  ...
  private void sendHelp(CommandSender sender) {
    ...
    if (!(admin || healNear))
      sender.sendMessage(
          getMessage("plugin-help-no-commands", null, sender.getName(), null, null, null, null));
    else {
      if (admin) {
        sender.sendMessage(GREEN + "/ch reload :" + PURPLE + " reloads the config from the file.");
        sender.sendMessage(
            GREEN
                + "/ch (creeper|TNT|ghast|custom|fire) (on/off) (world) :"
                + PURPLE
                + " toggle replacement");
        sender.sendMessage(
            GREEN + "/ch [on|off] (world) :" + PURPLE + " toggle CH for this world.");
        sender.sendMessage(
            GREEN
        ...
        sender.sendMessage(
            GREEN
                + "/ch (waitBeforeHealBurnt|wbhb) [seconds] :"
                + PURPLE
                + " Same for a burnt block");
        sender.sendMessage(
            GREEN
                + "/ch (blockPerBlockInterval|bpbi) [ticks] :"
                + PURPLE
      ...
      sender.sendMessage(
          GREEN
              + "/ch healNear"
              + (healNearOther ? " (player)" : "")
              + " :"
              + PURPLE
              + " Heals all explosions around"
   ...
   */
  private void booleanCmd(
      WorldConfig world, WCfgVal key, String[] args, String setting, CommandSender sender) {
    if (sender instanceof Player && !checkPermissions((Player) sender, "admin"))
      sender.sendMessage(
          getMessage("no-permission-command", null, sender.getName(), null, null, null, null));
   ...
   * @param unit The unit in the measurement.
   * @param sender The command's sender.
   */
  private void integerCmd(
      CfgVal key, String[] args, String help, String unit, CommandSender sender) {
    if (sender instanceof Player && !checkPermissions((Player) sender, "admin"))
      sender.sendMessage(
          getMessage("no-permission-command", null, sender.getName(), null, null, null, null));
   ...
   *
   * @param args The command arguments.
   * @param msg The name of the type of blocks.
   * @param sender The command's sender.
   * @param currentWorld The world to which the command must be applied. If null, apply to all
   *     worlds.
   */
  private void forceCmd(
    ...
    if (currentWorld == null)
      if (burnt) BurntBlockManager.forceReplaceBurnt();
      else ExplodedBlockManager.forceReplace();
    else if (burnt) BurntBlockManager.forceReplaceBurnt(currentWorld);
    else ExplodedBlockManager.forceReplace(currentWorld);
  ...
  private void healNear(CommandSender sender, String[] args) {
    ...
    } else sender.sendMessage(ChatColor.RED + "Please give the target's name.");
  }
  ...
  private void enable(boolean enable, WorldConfig currentWorld, CommandSender sender) {
    ...
    if (hasPerm) {
      if (currentWorld == null) {
        for (World w : Bukkit.getWorlds()) enable(enable, CreeperConfig.getWorld(w), sender);
        return;
      }
      currentWorld.setBool(WCfgVal.WORLD_ON, enable);
      sender.sendMessage(
          GREEN + (enable ? "En" : "Dis") + "abled CH in world : " + currentWorld.getName());
    }
  }
