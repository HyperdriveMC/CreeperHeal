...
public class CreeperPlayer {
   ...
   * Constructor.
   *
   * @param player The player represented by this class.
   */
  ...
  private void loadPermissions() {
    lava =
        CreeperPermissionManager.checkPermissions(player, true, "grief.warn.*", "grief.warn.lava");
    tnt = CreeperPermissionManager.checkPermissions(player, true, "grief.warn.*", "grief.warn.tnt");
    fire =
        CreeperPermissionManager.checkPermissions(player, true, "grief.warn.*", "grief.warn.fire");
    blacklist =
        CreeperPermissionManager.checkPermissions(
            player, true, "grief.warn.*", "grief.warn.blacklist");
    spawnEggs =
        CreeperPermissionManager.checkPermissions(
            player, true, "grief.warn.*", "grief.warn.spawnEggs");
   ...
   * @param cause The reason for the message.
   * @param message The message send.
   */
  public void warnPlayer(Player player, WarningCause cause, String message) {
    switch (cause) {
      case BLACKLIST:
        if (blacklist) player.sendMessage(message);
        break;
      case TNT:
        if (tnt) player.sendMessage(message);
        break;
      case FIRE:
        if (fire) player.sendMessage(message);
        break;
      case LAVA:
        if (lava) player.sendMessage(message);
        break;
      case SPAWN_EGG:
        if (spawnEggs) player.sendMessage(message);
        break;
      case PVP:
        if (pvp) player.sendMessage(message);
        break;
    }
  }
